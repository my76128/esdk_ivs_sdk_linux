...
static THREAD_HANDLE  g_hWriteThread = NULL;
#endif 
...
const static char* g_strLogType[] = {
    "security", 
    "operation",
    "run", 
    "interface",
...
const static char* g_strLogLevel[] = {
    "Emergency",
    "Alert    ",
    "Critical ",
    "Error    ",
...
#if defined MODULE_NETSOURCE_NAMESPACE
namespace NAMESPACE_IVS_NETSOURCE
{
#elif defined MODULE_PLAYER_NAMESPACE
namespace NAMESPACE_IVS_PLAYER
{
#endif
#endif
...
#else
	char tempDir[MAX_FNAME_LEN+20] = {0};
	sprintf(tempDir,"mkdir -p %s",g_LogRootDir);
	int ret = system(tempDir);
	printf("Create Log Root Directory[%s],ret=%d\n",tempDir,ret);	
	if (errno != 0)
	{
		printf("Log Error: Create Log Root Directory Fail\n");
	}

	if (0 > IVS_access(g_LogRootDir, 0))
#endif
    {
        printf("Log Error: Create Log Root Directory Fail\n");
        return -1;
