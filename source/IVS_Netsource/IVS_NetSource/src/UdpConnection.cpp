...
int CUdpConnection::Bind(const char* pszLocalIP, unsigned int uiLocalPort)
{
    ...
    {
        IVS_LOG(IVS_LOG_ERR, "Bind", "Create udp socket failed, error:%d",  CONN_ERRNO);
        return IVS_FAIL;
    }
    ...
    {
        IVS_LOG(IVS_LOG_ERR, "Bind", "Bind udp socket failed, error:%d",  CONN_ERRNO);
        return IVS_FAIL;
    }
...
int CUdpConnection::Connect(const char* pszServerIP, unsigned int uiServerPort)
{
	...
	{
		IVS_LOG(IVS_LOG_ERR, "Connect", "Connect udp socket failed, error:%d",  CONN_ERRNO);
...
int CUdpConnection::RecvData(char* pData, unsigned int uiDataLen)
{
    ...
    if (iRet <= 0)
    {
       // IVS_LOG(IVS_LOG_ERR, "RecvData", "Select read mode error, error code:%d", CONN_ERRNO);
    }
    else
    {
