...
// Rtsp媒体数据回调
static void  __SDK_CALL g_fRtspDataCallBack(long handle, char *pBuffer, unsigned int ulBufSize, void *pUser)//lint !e528 
{
...
// 下载裸码流回调;
static IVS_VOID __SDK_CALLBACK g_fDownLoadRawCallBack(IVS_RAW_FRAME_INFO* pstRawFrameInfo,
	IVS_CHAR*           pFrame,
	IVS_UINT32          uFrameSize,
	REC_PACKET_INFO*    /*pstRecPacketInfo*/,
	IVS_VOID*       pUser)
{
	CDownload* pDownload = static_cast<CDownload*>(pUser);
	if (NULL == pDownload)
	{
		return;
	}
	BP_RUN_LOG_INF("Start Platform DownLoad Frame CB", "Success.");
	pDownload->DoRawFrameCallBack(pstRawFrameInfo, pFrame, uFrameSize);
}//lint !e818 !e528
...
CDownload::CDownload(void)
{
	...
	m_fDownloadCallBackRaw = NULL;
	m_pDownloadUserDataRaw = NULL;
	m_pDownloadRawCBMutex = VOS_CreateMutex();
...
int CDownload::StartPlatformDownLoadRawCB(const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,
	...
	void* pUserData)
{
	...
	//增加裸码流回调
	VOS_MutexLock(m_pDownloadRawCBMutex);
	m_fDownloadCallBackRaw = pFPlatformDownLoadRawCB;
	m_pDownloadUserDataRaw = pUserData;
	VOS_MutexUnlock(m_pDownloadRawCBMutex);
	iRet = IVS_PLAY_SetRawFrameCallBack(m_ulPlayerChannel, &g_fDownLoadRawCallBack, this);
	BP_RUN_LOG_INF("Start Platform DownLoad CB", "Channel = %ul",m_ulPlayerChannel);
	//裸码流回调
...
int CDownload::StartPUDownLoadRawCB(
	const char* pCameraCode,
	const IVS_DOWNLOAD_PARAM* pDownloadParam,
	...
	void* pUserData)
{
	...
	if (stTimeSpanInter.iStartTime >= stTimeSpanInter.iEndTime)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "Start PU DownLoad RawCB", "time span invalid");
		return IVS_PARA_INVALID;
	}
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Start PU Download RawCB", "Start play error.");
		return iRet;
	}
	...
	//增加裸码流回调
	VOS_MutexLock(m_pDownloadRawCBMutex);
	m_fDownloadCallBackRaw = pFPUDownLoadRawCB;
	m_pDownloadUserDataRaw = pUserData;
	VOS_MutexUnlock(m_pDownloadRawCBMutex);
	iRet = IVS_PLAY_SetRawFrameCallBack(m_ulPlayerChannel, &g_fDownLoadRawCallBack, this);
	BP_RUN_LOG_INF("Start Platform DownLoad CB", "Channel = %ul",m_ulPlayerChannel);
...
void CDownload::DoRawFrameCallBack(IVS_RAW_FRAME_INFO* pstRawFrameInfo, IVS_CHAR* pFrame, IVS_UINT32 uFrameSize)
{
	VOS_MutexLock(m_pDownloadRawCBMutex);
	if (NULL != m_fDownloadCallBackRaw)
	{
		m_fDownloadCallBackRaw(m_ulHandle, pstRawFrameInfo, pFrame, uFrameSize, m_pDownloadUserDataRaw);
	}
	VOS_MutexUnlock(m_pDownloadRawCBMutex);
}
