...
// Rtsp媒体回调函数
void __SDK_CALL  RtspDataCallBack(long handle, char *pBuffer, unsigned int ulBufSize, void *pUser)
{
	return;
}//lint !e818
											...
											IVS_MEDIA_ADDR& stMediaAddrSrc)
{
	...
	// 建立RTSP连接
	iRet = RTSP_CLIENT_GetHandle(true, &m_nRtspHandle);
	if (IVS_SUCCEED != iRet || 0==m_nRtspHandle)
	{
	...
	iRet = RTSP_CLIENT_SetDataCallBack(m_nRtspHandle, RtspDataCallBack, this);
	iRet += RTSP_CLIENT_SendSetupMsg(m_nRtspHandle, &stServiceAddr, mediaRsp.szURL, RTSP_CLIENT_TIMEOUT, bTcpFlag, &stTPMCUAddr, &stMUMediaAddr);

	if (IVS_SUCCEED != iRet)
	{         
		(void)RTSP_CLIENT_SendTeardownMsg(m_nRtspHandle, mediaRsp.szURL, 1);
		(void)RTSP_CLIENT_FreeHandle(m_nRtspHandle);
		m_nRtspHandle = 0;
	...
	iRet = RTSP_CLIENT_SendPlayMsg(m_nRtspHandle, mediaRsp.szURL, dScale, &stMdieaRange, RTSP_CLIENT_TIMEOUT);
											...
											IVS_FLOAT fSpeed)
{
	...
	iRet = RTSP_CLIENT_SendPlayMsg(m_nRtspHandle, mediaRsp.szURL, (double)fSendScale, &stMdieaRange, RTSP_CLIENT_TIMEOUT);
...
IVS_INT32 CTPPlayChannel::StopPlayByIP()
{
	IVS_DEBUG_TRACE("");
	//关闭rtsp
	(void)RTSP_CLIENT_SendTeardownMsg(m_nRtspHandle, m_strURL.c_str(), 1);
	(void)RTSP_CLIENT_FreeHandle(m_nRtspHandle); 
	m_nRtspHandle = 0;
	m_strURL = "";
	return IVS_SUCCEED;
...
//暂停平台录像回放
IVS_INT32 CTPPlayChannel::PlatformPlayBackPauseByIP(void)
{
	IVS_DEBUG_TRACE("");
	//暂停rtsp
	IVS_INT32 iRet = RTSP_CLIENT_SendPauseMsg(m_nRtspHandle, m_strURL.c_str(), RTSP_CLIENT_TIMEOUT);
	if (IVS_SUCCEED != iRet)
	{
...
//恢复平台录像回放
IVS_INT32 CTPPlayChannel::PlatformPlayBackResumeByIP(void)
{
	IVS_DEBUG_TRACE("");
	//恢复rtsp
	if (RTSP_SESSION_STATUS_READY == RTSP_CLIENT_GetStatus(m_nRtspHandle))
	{
		...
		IVS_INT32 iRet = RTSP_CLIENT_SendPlayMsg(m_nRtspHandle, m_strURL.c_str(), (double)fSendScale, &stRange, RTSP_CLIENT_TIMEOUT);
		if (IVS_SUCCEED != iRet)
		{
...
int CTPPlayChannel::SetPlayBackSpeedByIP(IVS_FLOAT fSpeed)
{
	...
	RTSP_SESSION_STATUS state = RTSP_CLIENT_GetStatus(m_nRtspHandle);
		...
		RTSP_SESSION_STATUS_READY   == state)
	{
		...
		IVS_INT32 iRet = RTSP_CLIENT_SendPlayMsg(m_nRtspHandle, m_strURL.c_str(), (double)fSendScale, &stMdieaRange, RTSP_CLIENT_TIMEOUT);
		if (IVS_SUCCEED != iRet)
		{  
			(void)RTSP_CLIENT_SendTeardownMsg(m_nRtspHandle, m_strURL.c_str(), 1);
			(void)RTSP_CLIENT_FreeHandle(m_nRtspHandle);
...
IVS_INT32 CTPPlayChannel::SetPlayBackTimeByIP(IVS_UINT32 uiTime)
{
	...
	BP_RUN_LOG_INF("Set PlayBack Time ByIP", "RTSP_CLIENT_SendPlayMsg begin, rtsphandle=%d, speed=%f, settime=%u.", m_nRtspHandle, m_fRTSPSpeed, uiTime);
	...
	iRet = RTSP_CLIENT_SendPlayMsg(m_nRtspHandle, m_strURL.c_str(), (double)m_fRTSPSpeed, &stRange, RTSP_CLIENT_TIMEOUT);
