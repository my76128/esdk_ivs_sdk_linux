...
#ifndef WIN32
/***************************************************************************
 函 数 名 : GetModuleFilePath
 功能描述 : 获取指定库文件所在目录的上级目录
 输入参数 : sModuleName  库文件名称
               unSize  sPath长度
 输出参数 : sPath  库文件目录的上级目录
 返 回 值 : RET_OK-成功     RET_FAIL-失败
***************************************************************************/
...
//在redhat 32位平台下，使用acquire获取信号量一直超时
//解决办法：使用tryacquire方法，尝试获取5000次，5000次之后仍未获取到信号量，则按超时处理
#ifdef OS_REDHAT_i386
	int iTryCnt = 0;
	int nRet = 0;
